...
void print_distribution_multi_probe(const char* title, const char** nodes, int num_nodes, const char** keys, int num_keys, MultiProbeHashRing* hr) {
    printf("%s\n", title);
    ...
    for (int i = 0; i < num_keys; i++) {
        const char* assigned_node = get_multi_probe_node(hr, keys[i]);
        if (assigned_node) {
...
int main() {
    ...
    MultiProbeHashRing* hr = create_multi_probe_hash_ring(3, 3);
    if (!hr) {
        fprintf(stderr, "Failed to create Multi-Probe hash ring.\n");
        return 1;
    }
    ...
    for (int i = 0; i < num_initial_nodes; i++) {
        add_multi_probe_node(hr, initial_nodes[i]);
    }
    ...
    print_distribution_multi_probe("Initial distribution (3 replicas, 3 probes):", initial_nodes, num_initial_nodes, (const char**)keys, NUM_KEYS, hr);

    // Add a new node and observe re-distribution
    printf("\nAdding nodeD...\n");
    add_multi_probe_node(hr, "nodeD");
    const char* nodes_after_add[] = {"nodeA", "nodeB", "nodeC", "nodeD"};
    print_distribution_multi_probe("Distribution after adding nodeD:", nodes_after_add, 4, (const char**)keys, NUM_KEYS, hr);

    // Remove a node and observe re-distribution
    printf("\nRemoving nodeB...\n");
    remove_multi_probe_node(hr, "nodeB");
    const char* nodes_after_remove[] = {"nodeA", "nodeC", "nodeD"};
    print_distribution_multi_probe("Distribution after removing nodeB:", nodes_after_remove, 3, (const char**)keys, NUM_KEYS, hr);
    ...
    for (int i = 0; i < 5; i++) {
        ...
        const char* assigned_node = get_multi_probe_node(hr, key);
